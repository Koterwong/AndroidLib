/*
 * SmoothListView     2016/9/27-09-27
...
/**
 * Created by Koterwong on 2016/9/27 07:16
 */
public class SmoothListView extends ListView implements AbsListView.OnScrollListener {
  ...
  private final static int PULL_LOAD_MORE_DELTA = 50; // when pull up >= 50px  trigger load more.
  ...
  @Override public void setAdapter(ListAdapter adapter) {
    if (!mIsFooterReady) {  // make sure XListViewFooter is the last footer view, and only add once.
      mIsFooterReady = true;
  ...
  public void setLoadMoreEnable(boolean enable) {
    ...
    if (!mEnablePullLoad) {
      mFooterView.hideFooter();
      mFooterView.setOnClickListener(null);
      //make sure "pull up" don't show a line in bottom when listview with one page
      setFooterDividersEnabled(false);
    } else {
      mPullLoading = false;
      mFooterView.showFooter();
